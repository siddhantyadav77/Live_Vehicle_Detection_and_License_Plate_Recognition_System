...
def search_logs():
    ...
    try:
        ...
        log_text.insert(tk.END, f"{'Timestamp':<22} | {'Type':<10} | {'Plate':<10} | {'Dir':<6}\n")
        log_text.insert(tk.END, "-" * 60 + "\n")
        for entry in filtered[1:]:
            fields = entry.strip().split(',')
            if len(fields) >= 4:
                log_text.insert(tk.END, f"{fields[0]:<22} | {fields[1]:<10} | {fields[2]:<10} | {fields[3]:<6}\n")
...
tk.Button(search_frame, text="Search", command=search_logs, font=("Arial", 9), bg="#1e90ff", fg="white").pack(side="left", padx=5)
tk.Button(search_frame, text="Clear", command=clear_search, font=("Arial", 9), bg="#aaaaaa").pack(side="left", padx=5)
tk.Button(search_frame, text="Export Logs", command=export_logs, font=("Arial", 9), bg="#28a745", fg="white").pack(side="left", padx=5)
...
direction_label = tk.Label(info_frame, text="Direction: ", font=("Arial", 14), bg="#ffffff")
...
def detect_and_stream():
    global last_plate, last_vehicle_type, last_direction, last_image_path, plate_entry_record
    ...
    while True:
        ...
        for result in results:
            for box in result.boxes:
                ...
                if vehicle_type in ALLOWED_CLASSES:
                    ...
                    direction = "Out" if license_plate in plate_entry_record else "In"
                    if direction == "In":
                        plate_entry_record[license_plate] = True
                    else:
                        del plate_entry_record[license_plate]
                    ...
                    with open(log_file, "a", newline="") as f:
                        ...
                        writer.writerow([datetime.now(ist).strftime("%Y-%m-%d %H:%M:%S"),
                                         vehicle_type, license_plate, direction, image_path])
        ...
        cv2.putText(frame, f"Time: {timestamp_str}", (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 0), 2)
...
def update_last_captured():
    if last_image_path and os.path.exists(last_image_path):
        ...
        direction_label.config(text=f"Direction: {last_direction}")
